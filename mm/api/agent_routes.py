...
def get_pending_route(broker: TokenBroker, pending_id: str) -> PendingPollOutcome:
    """GET `/pending/{pending_id}`."""
    return broker.get_pending(pending_id)
...
def post_pending_route(
    ...
    if isinstance(body, ClarificationPostBody):
        return broker.post_clarification_response(pending_id, body.clarification_response)
    return broker.post_updated_request(pending_id, body.resource_token, body.justification)


def cancel_pending_route(broker: TokenBroker, pending_id: str) -> None:
    """DELETE `/pending/{pending_id}`."""
    broker.cancel_request(pending_id)
