#!/usr/bin/env python3
"""HWK-signed HTTP client for Person Server agent routes: POST /mission, POST /token, poll GET /pending/{id}.

Requires: aauth (project dependency), server with AAUTH_PS_INSECURE_DEV=false.

Examples:
  AAUTH_PS_INSECURE_DEV=false uvicorn ps.http.app:app --host 127.0.0.1 --port 8765
  # Default: POST /mission (built-in description) then POST /token with mission in body; poll on 202.
  ./scripts/hwk-ps-client.sh --base-url http://127.0.0.1:8765

  ./scripts/hwk-ps-client.sh --no-mission
  ./scripts/hwk-ps-client.sh --mission-description "# Trip\\n\\nPlan travel" --permission-action WebSearch
"""
...
# Shown when you run ./scripts/hwk-ps-client.sh with no args (mission + token in one flow).
_DEFAULT_MISSION_DESCRIPTION = """# HWK client demo

Default mission. Pass --mission-description to replace this, or --no-mission to skip POST /mission
and only POST /token with a resource token."""

_DEFAULT_MISSION_TOOLS: list[dict[str, str]] = [
...
def main() -> None:
    default_key = Path(__file__).resolve().parent / ".hwk-ps-client-key.pem"
    p = argparse.ArgumentParser(description="Person Server HWK client: mission, token, permission, audit, poll.")
    p.add_argument(
        "--base-url",
        default=os.environ.get("AAUTH_PS_BASE_URL", "http://127.0.0.1:8765"),
        help="Person Server base URL (no trailing path). Env: AAUTH_PS_BASE_URL",
    )
    p.add_argument(
        "--key-file",
        type=Path,
    ...
    p.add_argument(
        "--no-mission",
        action="store_true",
        help="Skip POST /mission; POST /token with only resource_token (no mission in JSON). Default is to POST /mission first with a built-in description.",
    )
    p.add_argument(
        "--mission-description",
        default=None,
        help="POST /mission with this Markdown (and optional --tools-json). If omitted, a built-in demo description is used unless --no-mission.",
    )
    p.add_argument(
        "--tools-json",
        default=None,
    ...
    p.add_argument(
        "--complete-mission",
        action="store_true",
        help="After token success, POST /interaction type=completion (needs a mission; not compatible with --no-mission).",
    )
    p.add_argument(
        "--poll-interval",
        type=float,
    ...
    if args.no_mission and (args.mission_description or args.tools_json):
        raise SystemExit("--no-mission cannot be combined with --mission-description or --tools-json")
    if args.no_mission and (args.permission_action or args.audit or args.complete_mission):
        raise SystemExit(
            "--no-mission is for token-only; omit it if you use --permission-action, --audit, or --complete-mission"
        )

    if not args.no_mission:
        if args.mission_description is not None:
            mdesc = args.mission_description
            ...
            mdesc = _DEFAULT_MISSION_DESCRIPTION
            tools = list(_DEFAULT_MISSION_TOOLS)
        body_m = {"description": mdesc, "tools": tools}
    ...
    print("\nDeferred consent required (202). Approve in the Person Server UI:", file=sys.stderr)
    if code:
